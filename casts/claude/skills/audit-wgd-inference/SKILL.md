...
description: "Referees a whole-genome-duplication inference against the known artifact axes; a claim that cannot clear an axis is flagged, never silently passed."
---
...
- Referees a whole-genome-duplication inference against the known artifact axes; a claim that cannot clear an axis is flagged, never silently passed.

## Load Upfront

- `references/notes/vanneste-2013-ks-saturation.md`: Research carried verbatim. Cardinal validity axis: Ks saturation can manufacture a spurious tail 'saturation peak' that mimics a WGD even with no WGD present; bounds where Ks dating / mixture inference is trustworthy.

## Load On Demand

- `references/notes/chen-2024-wgd.md`: Research carried verbatim. Reference pipeline + exact defaults (--kscutoff, node-weighting, mixture components) and the i-ADHoRe-not-MCScanX fact for method-identity checks. Use when: checking wgd/Ks-pipeline defaults, the Ks saturation cutoff, or a tool-identity claim (e.g. which collinearity engine wgd v2 uses).
- `references/notes/de-bie-2006-cafe.md`: Research carried verbatim. Grounds branch-attribution of an expansion burst and the multiple-testing caveat on family-level p-values. Use when: a gene-family expansion / birth-death rate-shift is offered as independent WGD corroboration.
- `references/notes/han-2013-cafe3.md`: Research carried verbatim. Guards against treating a gene-count expansion burst from a fragmented assembly as real WGD signal without an error model. Use when: a gene-family expansion is offered as WGD corroboration and the assembly/annotation quality is uncertain.
- `references/notes/lovell-2022-genespace.md`: Research carried verbatim. Synteny-anchored orthology with array-representative collapse of tandem arrays; blkSize=5 (unique hits, not orthogroups), synBuff=100, nGaps=5. Use when: synteny/orthology block-size params or array-collapse of tandem arrays are in question.
- `references/notes/mendes-2020-cafe5.md`: Research carried verbatim. Supports a converging-evidence check: an independent burst of gene-family expansions datable to the same node. Use when: a gene-family expansion / birth-death rate-shift is offered as independent WGD corroboration.
- `references/notes/qiao-2019-dupgen.md`: Research carried verbatim. Five duplication modes (WGD/tandem/proximal/transposed/dispersed); WGD = intra-genome collinear pairs; guards the 'segmental'/'tandem=5' mislabel. Use when: duplication-mode classification, or tandem/proximal contamination of a Ks peak, is at issue.
- `references/notes/sensalari-2022-ksrates.md`: Research carried verbatim. Cross-lineage synonymous-substitution-rate differences invalidate naive mixed-plot placement; RRT-style branch-decomposition correction is the remedy. Use when: the claim positions a WGD before/after a speciation, or compares Ks across differently-evolving lineages.
- `references/notes/tang-2008-jcvi.md`: Research carried verbatim. Syntenic depth/multiplicity as the ploidy index; unpurged tandems inflate multiplicity; depth must be read against an assumed per-lineage event history. Use when: a specific ploidy level (duplication vs triplication) or syntenic depth/multiplicity is claimed.
- `references/notes/wang-2012-mcscanx.md`: Research carried verbatim. Collinear-block thresholds (score>250 / >=5 pairs, gap<25, E<1e-5) and tandem=rank-diff-1 / proximal<20 definitions; WGD signal = anchors in collinear blocks. Use when: assessing synteny-anchor / collinear-block criteria or duplicate-class definitions.
