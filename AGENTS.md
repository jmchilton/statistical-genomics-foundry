...
Each source gets a short identifier and a directory `research/<paper|tutorial|books>/<id>/` — mirroring
...
tutorials** use `/summarize-source`, where short verbatim *load-bearing* quotes are allowed (open /
fair-use sources, and verbatim aids recoverability). Unifying the two specs is an open reconciliation item.
