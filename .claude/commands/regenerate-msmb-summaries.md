---
description: Regenerate MSMB chapter summaries in content/research/books/msmb/ from raw chapters
...
`NN` is the zero-padded number (`01`…`14`) used for the raw input filename. Raw input is `corpus-import/msmb/raw/NN-chap.html`; output is `content/research/books/msmb/chap<n>/index.md` (`<n>` = un-padded chapter number; the `<slug>` is kept only for the frontmatter `title`, not the path).
...
3. **You (the main session) write each file.** Wrap the returned body with this exact frontmatter + H1, then write `content/research/books/msmb/chap<n>/index.md`:
