...
description: "Separates technical variation from biology with explicit design checks, pinned tooling, reproducible adjustment, and a referee handoff."
---
...
- Separates technical variation from biology with explicit design checks, pinned tooling, reproducible adjustment, and a referee handoff.

## Load Upfront

- `references/notes/chap13.md`: Research carried verbatim. Design vocabulary for the frame + design-review phases: experiment taxonomy, noise-vs-bias, blocking/pairing/balance/randomization ('block what you can, randomize what you cannot'), effective sample size, and the rule that data already confounded at collection cannot be rescued by any analysis.
- `references/notes/chap8.md`: Research carried verbatim. The design matrix carries the model's scientific content; a known nuisance factor enters as a blocking term (at a degrees-of-freedom cost); PCA/heatmap/p-value-histogram diagnostics for an unmodelled batch factor.
- `references/notes/deseq2.md`: Research carried verbatim. The batch-in-design idiom (~ batch + condition, variable of interest last), the raw-counts input rule, and the mechanical aliasing detector: the exact error 'the model matrix is not full rank, so the model cannot be fit as specified.' plus its two stated remedies.
- `references/notes/leek-2010.md`: Research carried verbatim. The detection recipe (cluster/MDS/PC-vs-batch-surrogate), the claim that normalization does not remove batch effects, the adjustment menu (covariate / ComBat / SVA), and the boundary condition: adjustment works only when batch is not highly confounded with biology.
- `references/notes/nygaard-2016.md`: Research carried verbatim. The three-legged failure the doer must never build (unbalanced design + group-preserving batch removal + downstream analysis that ignores batch) and its endorsed remedy: account for batch in the statistical analysis.

## Load On Demand

- `references/notes/designit.md`: Research carried verbatim. Pinned allocation tooling (designit 0.5.0): BatchContainer / optimize_design / osat_score_generator signatures and every default; the exact assertion strings; the two latent 0.5.0 bugs. Version-pinned signatures must be carried verbatim — paraphrasing them is the failure. Use when: an assignment must actually be computed and a pinned, runnable allocation API is needed.
- `references/notes/kang-2018-demuxlet.md`: Research carried verbatim. Pooling + genotype-based demultiplexing as an established design-time route, with its hard input (external genotypes for every pooled donor), its operating point (50 SNPs/cell, ≥1000 UMIs/cell, unrelated donors), and its practical pool-size optimum (~20). Use when: the platform is droplet scRNA-seq and donors would otherwise each occupy their own run.
- `references/notes/leek-storey-2007-sva.md`: Research carried verbatim. What a surrogate variable is and how it is constructed (residualize on the primary variable → SVD → permutation test on eigengenes → build SVs on the ORIGINAL matrix); and the rule that SVs enter subsequent regressions as covariates with gene-specific coefficients — never subtracted from the data. Use when: the batch/technical variable is unmeasured or unrecorded.
- `references/notes/limma.md`: Research carried verbatim. The one first-party, author-written ASSERTION (not mere enactment) that a batch-corrected matrix must not be fed to the downstream linear model. limma 3.68.4 reference manual, removeBatchEffect Note. Use when: whenever a batch-corrected/'cleaned' expression matrix is produced, or a downstream test is about to consume one.
- `references/notes/munafo-2018-collider.md`: Research carried verbatim. Guards the one selection step this Mold really does own: choosing which samples are profiled at all. Use when: only a selected subset of a cohort is being assayed (e.g. samples chosen on DNA availability / data completeness).
- `references/notes/rnaseqgene.md`: Research carried verbatim. The only recovered worked code for the estimate→design handoff: svaseq(dat, mod, mod0, n.sv=2) on normalized counts, then design(ddssva) <- ~ SV1 + SV2 + dex; and the RUVg empirical-control-gene parallel. Use when: estimated factors (SVs / RUV factors) must be carried into a count-based DE design.
- `references/notes/sva.md`: Research carried verbatim. Pinned API (sva 3.60.0, Bioconductor 3.23): full signatures and defaults, the data-type split (sva=continuous, svaseq=counts, ComBat=normalized, ComBat_seq=raw counts), the SVs-in-BOTH-models rule, and the console detector strings. Use when: calling num.sv / sva / svaseq / ComBat / ComBat_seq.
- `references/notes/yan-2012-osat.md`: Research carried verbatim. The RCBD target, the constrained-allocation objective V = Σ_ij (n_ij − E_ij)², the χ² variable-vs-batch diagnostic, and the explicit caveat that design alone cannot eliminate batch effects. NOTE: the paper states no package version and no defaults — the API and its coded nSim=100 default come from osat (the pinned package docs), not from here. Use when: samples are already collected but not yet assigned to batches/plates/chips.
- `references/notes/zhang-2020-combat-seq.md`: Research carried verbatim. The NB/quantile-mapping count-adjustment method, its integer-preserving output, and — load-bearing — its own 'adjust only when batch effects are present and harmful' prescription plus the FPR 0.059–0.067 over-correction regime. Use when: an adjusted COUNT matrix (not a covariate) is required for a downstream tool.
...
method description a referee will judge** — and hand off. This skill cannot certify its own output.
...
- **Which moment we are in.** Pre-run (samples not yet allocated) and post-run (data in hand) are
  different skills' worth of work; do not silently switch.
...
**On an aliased design this skill HALTS.** It does not choose an adjustment method. It emits the finding
...
this skill has no sourced alternative.]
...
**This skill may not certify its own output.** Hand the following to `audit-batch-design-validity`:
...
Terminal states are the referee's, not this skill's: **CERTIFY**, **REVISE**, **ESCALATE**.
