...
description: "Decides whether a gene-loss call is an established loss or a technical artifact, working ordered gates in which silence is never taken as evidence."
---
...
- Decides whether a gene-loss call is an established loss or a technical artifact, working ordered gates in which silence is never taken as evidence.

## Load Upfront

- `references/notes/kirilenko-2023-toga.md`: Research carried verbatim. The intactness/loss classification substrate: five transcript states (intact / partially intact / missing / uncertain loss / lost), the M-vs-D assembly-gap safeguard (>=10 N), the middle-80% rule, the >=2-exon / <60%-intact lost criterion, orthology-score >=0.5, and the coding-capacity-not-expression scope.
- `references/notes/turakhia-2020-gene-loss.md`: Research carried verbatim. The validity-axis anchor: the enumerated technical causes of spurious loss (assembly gaps, annotation-tracks-N50, paralog-as-ortholog, private mutations, reference bias) and the guard for each (gap masking, raw-assembly not annotation, >=20x ortholog/paralog separation, gene-in-synteny >=20, unique 1:1, first/last-2-AA exclusion, phylogenetic filter), plus the >30%-vs-<10% FPR ablation that proves absence-of-annotation alone is not loss.

## Load On Demand

- `references/notes/armstrong-2020-cactus.md`: Research carried verbatim. The reference-free WGA substrate (HAL); grounds the reference-choice-bias check and that alignment accuracy degrades with divergence (F1 0.989 primate vs 0.795 mammal). Use when: reference-choice bias or the upstream whole-genome-alignment substrate is at issue.
- `references/notes/fiddes-2018-cat.md`: Research carried verbatim. Multi-method integration substrate (TransMap + AUGUSTUS variants + homGeneMapping over a Cactus/HAL alignment); grounds that projection is coverage-filtered and that input-annotation errors propagate across a clade. Use when: the claim comes from a multi-method / consensus annotation set (CAT).
- `references/notes/sharma-hiller-2017-cesar2.md`: Research carried verbatim. Establishes exon projection as an HMM codon/frame- and splice-site-aware step whose known hard case is shifted splice sites; grounds 'exon-projection failure' as a distinct artifact class separate from true deletion. Use when: an exon-projection or splice-site-shift artifact is at issue.
- `references/notes/shumate-salzberg-2021-liftoff.md`: Research carried verbatim. The mis-use guard: Liftoff is a same/closely-related-species annotation-transfer tool, not a loss detector; absence in its output is a failed lift-over reported to the -u unmapped file, and success is limited by divergence. Defaults -a 0.5 / -s 0.5. Use when: the claim rests on a Liftoff / minimap2 pairwise annotation-transfer run.
