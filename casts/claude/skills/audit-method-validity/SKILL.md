...
description: "Reasons about an analysis's method validity from its method + data-handling trace, emitting a verdict that gates certification — never CERTIFY on its own."
---
...
- Reasons about an analysis's method validity from its method + data-handling trace, emitting a verdict that gates certification — never CERTIFY on its own.
...
- `references/patterns/batch-aliased-with-condition.md`: Pattern carried verbatim. Rank-deficiency signature; the unfixable case that forces ESCALATE. Use when: a technical batch factor co-varies with the biological contrast.
- `references/patterns/double-dipping.md`: Pattern carried verbatim. Signature catalog + remedy (count-splitting / selection-aware null) for circular / post-selection inference. Use when: feature selection and the hypothesis test share the same data/cells/samples.
- `references/patterns/garden-of-forking-paths.md`: Pattern carried verbatim. Signature for undeclared researcher degrees of freedom. Use when: analytic choices were made after seeing the data; the test family is undeclared.
- `references/patterns/method-applicability-errors.md`: Pattern carried verbatim. Ground 'is this method real and appropriate?' in the two-prong applicability taxonomy. Appropriateness prong = corpus-observed (StatQA condition->method mapping); existence/invented-method prong = flagged white-space (no benchmark) per the leaf. Use when: the named method may not exist, or its assumptions may not match the data regime/design.

## Procedure

# Audit Method Validity

Referee — Family B / critique. **Input:** an analysis's *method description* + *data-handling trace*. **Output:** a structured validity verdict that gates certification. This skill *reasons about* validity; it does not run the empirical check. Findings that need empirical confirmation are emitted for the gate, not resolved here.
...
- No fatal findings → `CRITIQUE-CLEAR` — **not** CERTIFY. Critique is necessary, not sufficient; certification requires the calibrate pass the gate mandates (`REFEREE_LOOP` §5). **This skill never returns CERTIFY.**
